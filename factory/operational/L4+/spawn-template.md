# L4+ Workstream Composition Review Spawn Template

Filled by the harness when opening a workstream review.
...
Review the submitted workstream candidate at:

`{{WORKSTREAM_CANDIDATE_POINTER}}`

L3 workstream brief:

`{{WORKSTREAM_BRIEF_POINTER}}`

Frozen workstream rubric:

`{{WORKSTREAM_RUBRIC_POINTER}}`
...
- `reviews/{{GATE_ID}}/gate-composition-report.md`
...
- L3 workstream brief and inherited requirements;
- frozen workstream rubric;
- submitted workstream report;
- L4 plan/status;
- task briefs and acceptance pointers;
- task reports and local review reports;
...
When `reviews/{{GATE_ID}}/gate-composition-report.md` is complete, write the review seat's terminal
signal as `DONE` with `evidence.verdict` set to `ACCEPT`, `BOUNCE`, or
`ESCALATE` and `evidence.gate_id` set to `{{GATE_ID}}`. Use
`evidence.gate_artifact` for `reviews/{{GATE_ID}}/gate-composition-report.md`
