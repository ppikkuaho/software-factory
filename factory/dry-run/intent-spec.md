...
| Outcome ID | Outcome | Verbatim intent spans |
|---|---|---|
| O-001 | The user can sell products online and reliably collect money for orders, with Stripe as the payment processor. | "an e-commerce backend"; "must use Stripe" |
| O-002 | Running it stays cheap — hosting cost is a standing priority, not an afterthought. | "keeping hosting cost low" |
| O-003 | A customer is **never charged twice** for one purchase. This is the line that must never be crossed. | "never double-charge a customer" |

Slice framing: O-001 and O-003 are what the **Payments slice** delivers and guarantees. O-002 is a cross-cutting constraint that shapes every area, including this slice. The rest of "an e-commerce backend" (catalog, cart, fulfillment, accounts) is acknowledged scope but **out of this slice** and tagged `deferred`/out-of-slice below.
...
|----|-------------|-----|-----|--------|---------|-------|
| R-001 | System is an e-commerce backend: it accepts orders and collects payment for them. | decided | — | O-001 | plain | The product framing. Payments slice realizes the payment-collection half. |
| R-002 | Stripe is the payment processor; payment integration is built against Stripe. | decided | — | O-001 | technical | Hard user constraint. Not an option to weigh — fixed. User fluent at flow level. |
| R-003 | Hosting/running cost is kept low; cost is a first-class design constraint, weighed against other -ilities. | decided | — | O-002 | plain | User opinionated on the *goal* (cheap), delegated on the *means* (which infra). See tradeoff probe T-1. |
| R-004 | Architecture/stack choices (language, framework, datastore, deploy target) are left to professional judgment, subject to R-003 (cheap) and R-002 (Stripe). | delegated | — | O-002 | plain | User: "use your best judgment." Delegated WITH the two constraints above as the bounding rubric. |
| R-005 | Payment flow: customer submits an order → backend creates a charge via Stripe → order is marked paid on success. | decided | — | O-001 | technical | User fluent at this altitude; co-described the flow. |
...
| **R-007** | **A customer is never double-charged for a single purchase.** | **decided** | **YES** | **O-003** | **plain** | **THE must-never-fail.** Decomposed to atomic obligations R-007.1 / R-007.2 / R-007.3 below. User confirms the *decomposition*, not just the outcome. |
...
| R-008 | On a charge failure or ambiguous outcome, the system never silently leaves the customer charged-but-order-unpaid or paid-but-uncharged (no money/order divergence). | delegated | — | O-003 | plain | Adjacent integrity obligation surfaced while decomposing R-007. NOT what the user literally said; raised at reflect-back. Tagged delegated because user delegated "everything else," but flagged because it is in the blast radius of "never double-charge." |
| R-009 | Catalog / product management. | deferred | — | O-001 | plain | Out of Payments slice. Part of "an e-commerce backend." Revisit when slice gates. |
| R-010 | Cart / checkout assembly (pre-payment). | deferred | — | O-001 | plain | Out of slice. The Payments slice assumes an order arrives ready-to-charge. |
| R-011 | Customer accounts / auth. | deferred | — | O-001 | plain | Out of slice. |
| R-012 | Fulfillment / order lifecycle past "paid." | deferred | — | O-001 | plain | Out of slice. |
| R-013 | Refunds / disputes / partial captures. | deferred | — | O-003 | technical | Out of slice but adjacent to R-007 — a refund path can re-open double-charge risk. Explicitly parked, not dropped. |
...
These rows are played back with the requirement claims and become `confirmed`
only when the user confirms the reflect-back. They name the recipient-visible
paths the L2+ product probe must later drive; downstream reviewers do not infer
additional journeys from the Outcomes prose.

| Journey ID | Kind | Requirement IDs | Starting state | Action through the face | Observable result | MNF obligation | Reflect-back status |
|---|---|---|---|---|---|---|---|
| J-001 | positive | R-001, R-002, R-005 | one valid unpaid order and Stripe test credentials | submit the documented payment request once | exactly one Stripe charge is created and the order becomes paid | — | pending |
| J-002 | negative | R-007.1 | the same purchase was already submitted and its request identity is retained | replay the same payment request after a timeout | the replay is rejected or coalesced and no second charge is created | R-007.1 | pending |
| J-003 | negative | R-007.2 | one Stripe event was already consumed | deliver the same documented Stripe event ID again | the duplicate is a no-op and causes no second state transition | R-007.2 | pending |
| J-004 | negative | R-007.3 | one unpaid purchase identity is visible to two concurrent clients | submit both documented payment requests concurrently | exactly one charge succeeds; the other request is rejected or coalesced | R-007.3 | pending |
